...
function CopyField({ value, label, fallback }: { value: string; label: string; fallback: string }) {
...
export function NewMessengerChannelForm({
  action,
  webhookUrl,
  verifyToken,
}: {
    ...
    <div className="space-y-6">
      <Card className="border-white/10 bg-card/60">
        <CardHeader>
          ...
          <CardDescription>
            Meta App Dashboard → Webhooks → Messenger → Subscribe. Paste these
            two values, then click <em>Verify and save</em>. This is a one-time
            app-level step (you only do it once, not per page).
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <CopyField label="Callback URL" value={webhookUrl} fallback="(unavailable)" />
          <CopyField
            label="Verify token"
            value={verifyToken}
            fallback="(set MESSENGER_WEBHOOK_VERIFY_TOKEN in env)"
          />
        </CardContent>
      </Card>
      ...
      <Card className="border-white/10 bg-card/60">
        <CardHeader>
          <CardTitle className="text-base">2 — Page credentials</CardTitle>
          <CardDescription>
            From Meta App Dashboard → Messenger → Settings → Access Tokens:
            generate a Page access token for the Page you want to connect.
            Xyra subscribes the Page to the webhook automatically.
          </CardDescription>
        </CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-white/50">
                Page → About → Page transparency, or via Graph API Explorer.
              </p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-white/50">
                ...
                in the channels table. Needs the <code>pages_messaging</code>{" "}
                permission.
              </p>
            </div>
          </CardContent>
